...
const initialState: LibraryState = {
  user: null,
  users: [],
  books: sampleBooks, // Initialize with sample books
  issues: [],
  fines: [],
  stats: {
    totalBooks: 0,
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        // Ensure we always have sample books if no books exist
        if (!parsedData.books || parsedData.books.length === 0) {
          parsedData.books = sampleBooks;
        }
        dispatch({ type: 'LOAD_DATA', payload: parsedData });
      } catch (error) {
        console.error('Error loading saved data:', error);
        // Load default data if parsing fails
        dispatch({ type: 'LOAD_DATA', payload: getDefaultData() });
      }
    } else {
